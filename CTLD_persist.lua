local myLog = mist.Logger:new('cltd')
--[[
Combat Troop and Logistics DropDrop
...
ctld.maximumDistanceLogistic = 700 -- max distance from vehicle to logistics to allow a loading or spawning operation
...
ctld.pickupZones = {
    ...
    { "USA Carrier", "blue", -1, "yes", 0, 1001 }, -- instead of a Zone Name you can also use the UNIT NAME of a ship
}
...
local humans = mist.utils.deepCopy(mist.DBs.humansByName)
for k,v in pairs (humans) do
    table.insert(ctld.transportPilotNames,v["unitName"])
end

myLog:msg("ctld transport pilot names:")
myLog:msg(ctld.transportPilotNames)
...
ctld.spawnableCrates = {
    ...
    ["Ground Forces"] = {
        ...
        { weight = 450, desc = "2A18M", unit = "2A18M" },
        ...
        { weight = 401, desc = "FOB Crate - Small", unit = "FOB-SMALL" }, -- Builds a FOB! - requires 3 * ctld.cratesRequiredForFOB
    },
    ["AA Crates"] = {
...
function ctld.spawnCrate(_arguments)
    ...
    local _status, _err = pcall(function(_args)
        ...
        if _crateType ~= nil and _heli ~= nil then --and ctld.inAir(_heli) == false
...
function ctld.generateTroopTypes(_side, _countOrTemplate, _country)
    ...
    if type(_countOrTemplate) == "table" then
        ...
        if _countOrTemplate.mg then
            _troops = ctld.insertIntoTroopsArray("Soldier M4 GRG",_countOrTemplate.mg,_troops)
        end
...
if not ctld.spawnedCratesBLUE then
...
ctld.hoverStatus = {} -- tracks status of a helis hover above a cratecrateLookupTable
